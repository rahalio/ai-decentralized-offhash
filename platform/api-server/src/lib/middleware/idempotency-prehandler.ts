...
import type { IdempotencyStore } from '@offhash/services/_shared';
  ...
  if (!key) {
    if (config?.requireIdempotency) {
      reply.status(400).type('application/problem+json').send({
        type: 'https://offhash.local/problems/IDEMPOTENCY_KEY_REQUIRED',
        title: 'Bad Request',
        status: 400,
  ...
  if (existing) {
    if (existing.bodyHash !== hash) {
      reply.status(409).type('application/problem+json').send({
        type: 'https://offhash.local/problems/IDEMPOTENCY_KEY_CONFLICT',
        title: 'Conflict',
        status: 409,
